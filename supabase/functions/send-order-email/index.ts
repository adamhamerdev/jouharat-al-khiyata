...
serve(async (req) => {
  ...
  try {
    const { product_name, customer_name, phone, quantity, order_id, variant_selected } = await req.json()

    const variantRow = variant_selected ? `
      <tr>
        <td style="padding: 12px 16px; color: #b05070; border-bottom: 1px solid #f0ccd5; width: 35%;">الاختيار</td>
        <td style="padding: 12px 16px; color: #7a1a3a; font-weight: bold; border-bottom: 1px solid #f0ccd5;">${variant_selected}</td>
      </tr>` : ''
      ...
      <div dir="rtl" style="font-family: Arial, sans-serif; max-width: 520px; margin: 0 auto; padding: 24px; background: #FDF0F3; border-radius: 16px;">
        ...
        <table style="width: 100%; border-collapse: collapse; background: #fff; border-radius: 12px; overflow: hidden; box-shadow: 0 2px 8px rgba(122,26,58,0.08);">
          <tr style="background: #7a1a3a;">
            <th colspan="2" style="color: #f5d78e; padding: 14px 16px; text-align: right; font-size: 15px; font-weight: 700;">تفاصيل الطلب</th>
          </tr>
          <tr>
